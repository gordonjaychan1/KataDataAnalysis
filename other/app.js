    ...
    <div>
      ...
      <h3 class="compare-head">Findings</h3>
        <ul style="font-size:13px;color:var(--text-muted);line-height:2.2;padding-left:20px">
          ...
          <li>Using the standard IQR method (scores below Q1 − 1.5×IQR or above Q3 + 1.5×IQR), male kata performance scores have <strong style="color:var(--text)">${mOut.low.length}</strong> low ${mOut.low.length === 1 ? "outlier" : "outliers"}${mOut.low.length ? ": " + fmtOutList(mOut.low) : ""} and <strong style="color:var(--text)">${mOut.high.length}</strong> high ${mOut.high.length === 1 ? "outlier" : "outliers"}${mOut.high.length ? ": " + fmtOutList(mOut.high) : ""}. Female kata scores have <strong style="color:var(--text)">${fOut.low.length}</strong> low ${fOut.low.length === 1 ? "outlier" : "outliers"}${fOut.low.length ? ": " + fmtOutList(fOut.low) : ""} and <strong style="color:var(--text)">${fOut.high.length}</strong> high ${fOut.high.length === 1 ? "outlier" : "outliers"}${fOut.high.length ? ": " + fmtOutList(fOut.high) : ""}. The female dataset contains more extreme lows, while male scores cluster more tightly at the bottom but reach higher peaks at the top.</li>
  ...
  if (kf) kf.innerHTML = `
    <div class="finding-block" style="margin-top:0">
      ...
      <ul style="font-size:13px;color:var(--text-muted);line-height:2.2;padding-left:20px">
        ...
        ${lowestDiff && highestDiff ? `<li>An interesting pattern emerges when comparing the <em>kata differential</em> (how much athletes score on a kata relative to their personal average) to win rate. <strong style="color:var(--text)">${esc(lowestDiff.Kata)}</strong> had the lowest differential (<strong style="color:var(--text)">${lowestDiff.Diff >= 0 ? "+" : ""}${lowestDiff.Diff.toFixed(3)}</strong>), meaning athletes performing it scored well below their personal average, so you would assume it has a low win rate. And it does; its win rate is <strong style="color:var(--text)">${(lowestDiff.Win_Rate * 100).toFixed(1)}%</strong>. However, you might assume that the kata with the highest differential, which is <strong style="color:var(--text)">${esc(highestDiff.Kata)}</strong> (<strong style="color:var(--text)">+${highestDiff.Diff.toFixed(3)}</strong>), would have a high win rate, but athletes scored <em>above</em> their personal average performing it yet its win rate was only <strong style="color:var(--text)">${(highestDiff.Win_Rate * 100).toFixed(1)}%</strong>.</li>` : ""}
