...
const I18N = {
  ...
  "howto.tables": { jp: `<strong>表の見方</strong>
                <p>列見出しをクリックすると並べ替え、もう一度で逆順になります。見出しにカーソルを合わせると説明が表示されます。詳細カードでは、統計値の下の小さな数字（例：<em>3/42</em>）が全体での順位を表し、<em>(T)</em> は同順位を示します。主要な表の下部には各列の平均を示す<strong>平均</strong>行が固定されています。各型には階級（上級・中級・初級）を示す色付きバッジが付きます（USA Karate型リストに基づく。一覧は参考資料タブの図 App-2）。</p>
                <p class="how-to-q"><em>並べ替え・順位の読み方・型の階級を知りたいときは？</em></p>` },
  "howto.links": { jp: `<strong>赤いリンクとナビゲーション</strong>
                <p><span style="color:var(--red);font-weight:600">赤色</span>で表示される型・選手・大会・国の名前はリンクです。クリックするとその項目の詳細カードへ移動します（確認プロンプトが表示されます）。詳細カード内・分析タブの表・検索結果など、どこでも機能します。</p>
                <p class="how-to-q"><em>選手のプロフィールから演武した型へ、または型からその型で最高得点を出した選手へ、どう移動する？</em></p>` },
  "howto.genderToggle": { jp: `<strong>男子／女子の切り替え</strong>
                <p>右上の<strong>男子</strong>／<strong>女子</strong>ボタンで、すべての表・グラフ・統計が即座に切り替わります。ボタンにカーソルを合わせると、その性別のシーズン概要が表示されます。図は性別ごとに番号付けされ、正確に引用できます（男子 <em>K-1, A-1, …</em>、女子 <em>FK-1, FA-1, …</em>）。</p>
                <p class="how-to-q"><em>男子の型の結果を見たい？それとも女子の型の結果を見たい？</em></p>` },
  "howto.search": { jp: `<strong>全体検索バー</strong>
                <p>右上の検索バーは、型・選手・大会・国を一度に横断検索します。入力すると結果が表示され、クリックするとその詳細カードへ移動します。どのタブからでも利用できます。</p>
                <p class="how-to-q"><em>探している型や選手の名前は分かっている。どうすれば素早くたどり着ける？</em></p>` },
  "howto.back": { jp: `<strong>戻るボタンとナビゲーション履歴</strong>
                <p>赤いリンクをたどると、左下に<strong>← 戻る</strong>ボタンが表示され、前のカードやタブに戻れます。起点まで戻ったとき、またはタブを手動でクリックすると消えます。</p>
                <p class="how-to-q"><em>いくつものリンクされたカードをたどって、戻りたい。戻るボタンはどこ？</em></p>` },
  "howto.dataKata": { jp: `<strong>型（Kata）</strong>
                <p>型ごとの統計：演武数・選手数・平均・中央値・最小／最大・標準偏差・勝率、そして選手個人の平均と比べたスコア。行をクリックすると、スコア分布のヒストグラム、演武した全選手、男女共通型のスコア比較を含む<strong>詳細カード</strong>が開きます。</p>
                <p class="how-to-q"><em>特定の型の統計は？誰が最も上手く演武している？</em></p>` },
  "howto.dataAthletes": { jp: `<strong>選手（Athletes）</strong>
                <p>選手ごとの統計：国・勝率・総合的な<strong>差（差分）</strong>（自分のスコアから相手のスコアを引き全試合で平均）など。行をクリックすると、順位付きのシーズン統計、スコアのヒストグラム、型のレパートリー、全試合の時系列ログ、対戦成績、メダルを含む<strong>詳細カード</strong>が開きます。</p>
                <p class="how-to-q"><em>特定の選手はシーズンを通じてどう戦った？最も多く対戦したのは誰？</em></p>` },
  "howto.dataTournaments": { jp: `<strong>大会（Tournaments）</strong>
                <p>大会ごとに1行（開催順）：演武数・選手数・型数・参加国数・平均スコア。上部の日程表から9大会のいずれへも移動できます。行をクリックすると、メダリストとその大会の全選手・型・国を含む<strong>詳細カード</strong>が開きます。</p>
                <p class="how-to-q"><em>特定の大会には誰が出場し、誰がメダルを獲得した？</em></p>` },
  "howto.dataCountries": { jp: `<strong>国（Countries）</strong>
                <p>国ごとの統計：選手数・演武数・大会数・平均／最高スコア・勝率・メダル数。<strong>世界地図</strong>は選手数で各国を色分けし（赤が濃いほど選手が多い）、クリックで開けます。行をクリックすると、所属選手・演武された型・出場大会（派遣選手数とメダル付き）・メダル集計を含む<strong>詳細カード</strong>が開きます。</p>
                <p class="how-to-q"><em>最も多く出場した国は？特定の国はシーズンを通じてどう戦った？</em></p>` },
  "howto.findKata": { jp: `<strong>型の分析（Kata Analysis）</strong>
                <p>型の傾向のグラフ：人気・平均スコア・勝率・階級別内訳、そして各型が選手個人の基準と比べてどう得点するか（選手の通常より「難しい」か「易しい」かの目安）。グラフ上部の文章がシーズンの注目すべき型の話題をまとめています。</p>
                <p class="how-to-q"><em>選手が好成績を収めている型は？トップ選手が最も選ぶ型は？</em></p>` },
  "howto.findAthlete": { jp: `<strong>選手の分析（Athlete Analysis）</strong>
                <p>平均スコアと勝率による選手ランキング、スコアと勝率の散布図、国別の内訳。各バーはクリックで詳細カードへ移動します。グラフ上部の<strong>アスリート・スポットライト</strong>が際立った選手を紹介します。</p>
                <p class="how-to-q"><em>最高得点の選手は誰？どの国が圧倒した？</em></p>` },
  "howto.compare": { jp: `<strong>男子 vs 女子</strong>
                <p>男女の並列比較：各性別だけが演武した型（表）、重なりを示すベン図、性別ごとの人気上位5型、そして男女共通型の「男子 − 女子」平均スコア差を示す棒グラフと並べ替え可能な表。</p>
                <p class="how-to-q"><em>片方の性別だけが演武する型はある？男女で最も得点が異なる型は？</em></p>` },
  "howto.medals": { jp: `<strong>メダル（Medals）</strong>
                <p>男子型と女子型のメダル表を並べて表示（既定は金メダル順）。全9大会を通じた各国の金・銀・銅・合計を示します。見出しをクリックで並べ替え。</p>
                <p class="how-to-q"><em>今シーズン最も多くメダルを獲得した国は？</em></p>` },
  "howto.notesCard": { jp: `<strong>注記（Notes）</strong>
                <p>データの収集方法、欠損データとその理由、型の階級の定義、WKFの採点規則、勝率の読み方。不明な点や予想外の点があれば、まずここをお読みください。</p>
                <p class="how-to-q"><em>このデータはどう収集された？統計は実際に何を意味する？</em></p>` },
  "howto.aboutCard": { jp: `<strong>概要（About）</strong>
                <p>制作者の背景とプロジェクトの動機、手法に関する注記、そしてサイト構築に使用した生CSVデータのダウンロードリンク。</p>
