...
const I18N = {
  ...
  /* ── Findings "Season Leaders" stat cards ── */
  "lead.title":            { en: "Season Leaders", jp: "シーズンリーダー" },
  "lead.note":             { en: "Score & win-rate leaders require more than 5 performances.", jp: "スコアと勝率のリーダーは5回を超える演武がある選手のみが対象です。" },
  "lead.tieNote":          { en: "Where leaders tie, the number sharing the top value is shown instead of a name.", jp: "首位が同率の場合は、名前の代わりに同率となった数を表示しています。" },
  "lead.tiedKata":         { en: "{n} kata tied", jp: "{n}型が同率" },
  "lead.tiedAthletes":     { en: "{n} athletes tied", jp: "{n}選手が同率" },
  "lead.mostPerformed":    { en: "Most Performed", jp: "最多演武" },
  "lead.mostPerformers":   { en: "Most Unique Performers", jp: "最多演武者数" },
  "lead.highestAvg":       { en: "Highest Avg Score", jp: "最高平均スコア" },
  "lead.highestWR":        { en: "Highest Win Rate", jp: "最高勝率" },
  "lead.highestSingle":    { en: "Highest Single Score", jp: "最高単一スコア" },
  "lead.mostConsistent":   { en: "Most Consistent", jp: "最も安定" },
  "lead.mostPerformances": { en: "Most Performances", jp: "最多演武" },
  ...
  "unit.stdDev":           { en: "std dev", jp: "標準偏差" },
  ...
  "howto.tables": { jp: `<strong>表の見方</strong>
                ...
                <p class="how-to-q"><em>データ表の並べ替えと読み方は？</em></p>` },
  "howto.detailCards": { jp: `<strong>詳細カード</strong>
                <ul>
                  <li>データ表の行をクリックすると、その項目の<strong>詳細カード</strong>（詳しいプロフィール）が開きます。</li>
                  <li>統計値の下の小さな数字（例：<em>3/42</em>）が全体での順位を表し、<em>(T)</em> は同順位を示します。</li>
                  <li>内容はタブごとに異なります：<strong>型</strong>カードは演武した全選手を、<strong>選手</strong>カードは型のレパートリー・全試合の時系列ログ・対戦成績・メダルを、<strong>大会</strong>カードはメダリストと出場した全員を、<strong>国</strong>カードは所属選手・型・出場大会を示します。</li>
                </ul>
  ...
  "howto.fiveTabs": { jp: `<strong>5つのタブ</strong>
                <ul>
                  <li><strong>型</strong> — 型ごとの統計：演武数・平均スコア・勝率・演武した選手数（実数）とその選手のその型での平均スコアなど。</li>
                  <li><strong>選手</strong> — 選手ごとの統計：演武数・平均スコア・勝率・相手との平均スコア差・演武した型（およびそれぞれの平均スコア）・全対戦相手（およびそれぞれへの勝率）など。</li>
                  <li><strong>大会</strong> — 大会ごとの統計：参加国・メダリスト・演武された型など。上部の<strong>日程表</strong>で2024-25シーズンを一覧・移動できます。</li>
                  <li><strong>国</strong> — 国ごとの統計：選手数・メダル・演武数など。<strong>世界地図</strong>は選手数で各国を色分けし（赤が濃いほど選手が多い）、国をクリックで開けます。</li>
                  <li><strong>メダル</strong> — 男子型と女子型のメダル表を並べて表示し、全9大会を通じた各国の金・銀・銅・合計を示します。</li>
                </ul>
                <p class="how-to-q"><em>5つのデータタブはそれぞれ何を示す？</em></p>` },
  "howto.findKata": { jp: `<strong>型の分析</strong>
                <ul>
                  <li>上部の<strong>シーズンリーダー</strong>欄が各指標で首位の型を紹介します — 最多演武・最多演武者・最高平均スコア・最高勝率・最高単独スコア・最も安定。各ボックスには固有のアクセントカラーがあり、複数の型が首位で同率の場合は名前の代わりに同率数を表示します。</li>
  ...
  "howto.findAthlete": { jp: `<strong>選手の分析</strong>
                <ul>
                  <li>上部の<strong>シーズンリーダー</strong>欄が各指標で首位の選手を紹介します — 最多演武・出場大会数・メダル・最高平均スコア・最高勝率・最高スコア差。各ボックスには固有のアクセントカラーがあり、選手が首位で同率の場合は同率数を表示します（ホバーすると同率の全選手が表示されます）。</li>
